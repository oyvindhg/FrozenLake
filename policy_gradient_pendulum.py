...
class policy_net():
    def __init__(self, learning_rate, n_states, n_actions, hidden_layers, dropout_rate):
...
class value_net():
    def __init__(self, learning_rate, n_states, hidden_layers):

        # Build the neural network
        self.input = tf.placeholder(shape=[None, n_states], dtype=tf.float32)
        hidden = slim.stack(self.input, slim.fully_connected, hidden_layers, activation_fn=tf.nn.relu)
...
def run(env, learning_rate, n_states, n_actions, hidden_layers, dropout_rate, total_episodes, max_steps, ep_per_update, gamma, group_size):
    ...
    baseline = value_net(learning_rate, n_states, hidden_layers)
